...
@Repository
public interface PeerHelpTutorProfileRepository extends JpaRepository<TutorProfile, Long> {
    ...
    @Query("SELECT tp FROM PeerHelpTutorProfile tp WHERE tp.subject.id = :subjectId AND tp.isAvailable = true ORDER BY tp.credibilityScore DESC")
    List<TutorProfile> findAvailableTutorsBySubjectOrderByCredibility(@Param("subjectId") Long subjectId);
    
    @Query("SELECT tp FROM PeerHelpTutorProfile tp WHERE tp.subject.id = :subjectId AND tp.isAvailable = true AND tp.proficiencyLevel >= :minProficiency ORDER BY tp.credibilityScore DESC")
