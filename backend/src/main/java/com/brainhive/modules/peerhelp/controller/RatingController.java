...
public class RatingController {
    ...
    /**
     * Quick rate a tutor from discovery page (Student only).
     */
    @PostMapping("/tutor/{tutorId}/quick")
    public ResponseEntity<ApiResponse<RatingResponseDTO>> createQuickTutorRating(
            @PathVariable Long tutorId,
            @Valid @RequestBody TutorQuickRatingDTO dto,
            HttpSession session) {
        try {
            User currentUser = userService.getCurrentUser(session);
            if (currentUser == null) {
            ...
            if (currentUser.getRole() != UserRole.STUDENT) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body(ApiResponse.error("Only students can rate tutors"));
            }

            RatingResponseDTO rating = ratingService.createTutorQuickRating(currentUser.getId(), tutorId, dto);
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(ApiResponse.success("Tutor rating submitted successfully", rating));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(ApiResponse.error(e.getMessage()));
        }
