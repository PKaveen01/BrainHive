...
const UploadResource = () => {
    ...
    const getDatabaseUserId = (authUser) => {
        ...
        // Read userId stored by auth.service.js after login
    ...
    // Takes userId directly so it doesn't depend on user state being set yet
    const fetchBookmarkStatus = async (resources, userId) => {
        try {
            const bookmarkStatus = {};

            for (const resource of resources) {
                try {
                    const response = await api.get(`/resources/${resource.id}/bookmarked/status?userId=${userId}`);
    ...
    const fetchAllUploads = async (userId) => {
        try {
            ...
            await fetchBookmarkStatus(uploadsData, userId);
